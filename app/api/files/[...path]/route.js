...
export async function GET(_request, context) {
  try {
    ...
    const employee = await Employee.findOne({ orgId: identity.orgId, photo: fileName }).select("_id photoFileId").lean();
    ...
    const stored = employee.photoFileId ? await readFromGridFS(employee.photoFileId) : null;
    const fileBuffer = stored?.buffer || await fs.readFile(path.join(EMPLOYEE_UPLOAD_DIR, fileName));
    return new Response(fileBuffer, { headers: { "Content-Type": stored?.file?.contentType || contentType, "Cache-Control": "private, max-age=3600", "X-Content-Type-Options": "nosniff" } });
  } catch (error) {
