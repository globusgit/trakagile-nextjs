...
export async function GET() {
  try {
    ...
    return Response.json({ user: { id: identity.userId, empId: identity.empId, orgId: identity.orgId, role: identity.role, name: employee?.name || identity.empId, email: employee?.email, photo: employee?.photo, isFirstLogin: identity.isFirstLogin, tokenVersion: identity.tokenVersion } });
