...
import { deleteFromGridFS, readFromGridFS } from "@/lib/gridfs";
...
export async function GET(_request, { params }) {
  try {
    ...
    const stored = document.gridFsFileId ? await readFromGridFS(document.gridFsFileId) : null;
    const bytes = stored?.buffer || await fs.readFile(path.join(DOCUMENT_UPLOAD_DIR, path.basename(document.storedName)));
    return new Response(bytes, { headers: { "Content-Type": stored?.file?.contentType || document.mimeType, "Content-Disposition": `inline; filename="${document.originalName.replaceAll('"', "")}"`, "Cache-Control": "private, no-store" } });
...
export async function DELETE(_request, { params }) {
  try {
    ...
    if (document.gridFsFileId) await deleteFromGridFS(document.gridFsFileId);
    else if (document.storedName) await fs.unlink(path.join(DOCUMENT_UPLOAD_DIR, path.basename(document.storedName))).catch((error) => { if (error.code !== "ENOENT") throw error; });
