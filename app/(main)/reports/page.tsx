...
type Report = { employees: Employee[]; rows: Row[]; summary: { records: number; workedMinutes: number; breakMinutes: number; overtimeMinutes: number; tripExpenses: number; lateArrivals: number; earlyDepartures: number } };
...
const currency = (value = 0) => new Intl.NumberFormat("en-IN", { style: "currency", currency: "INR", maximumFractionDigits: 2 }).format(value);
...
export default function ReportsPage() {
  ...
  return <div className="space-y-5 pb-10"><PageHeader title="Attendance Reports" />
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">{[["Records", report?.summary.records || 0], ["Worked", hours(report?.summary.workedMinutes)], ["Breaks", hours(report?.summary.breakMinutes || 0)], ["Late arrivals", report?.summary.lateArrivals || 0], ["Early departures", report?.summary.earlyDepartures || 0], ["Trip expenses", currency(report?.summary.tripExpenses || 0)]].map(([label, value]) => <Card key={label}><CardContent className="p-4"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></CardContent></Card>)}</div>
    <Card><CardHeader><CardTitle className="text-base">Monthly attendance</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full min-w-260 text-sm"><thead><tr className="border-b text-left">{["Employee", "Date", "Mode", "Mark in", "Mark out", "Worked", "Break", "Late", "Early", "Overtime", "Distance", "Trip expense"].map((heading) => <th key={heading} className="p-3 font-medium">{heading}</th>)}</tr></thead><tbody>{report?.rows.length ? report.rows.map((row) => <tr key={row.id} className="border-b"><td className="p-3"><b>{row.employeeName}</b><small className="block text-muted-foreground">{row.employeeId}</small></td><td className="p-3">{row.date}</td><td className="p-3">{row.attendanceType?.replaceAll("_", " ")}</td><td className="p-3">{time(row.markIn)}</td><td className="p-3">{time(row.markOut)}</td><td className="p-3">{hours(row.workedMinutes)}</td><td className="p-3">{row.breakMinutes ? `${row.breakMinutes}m` : "—"}</td><td className="p-3">{row.lateMinutes ? `${row.lateMinutes}m` : "—"}</td><td className="p-3">{row.earlyMinutes ? `${row.earlyMinutes}m` : "—"}</td><td className="p-3">{row.overtimeMinutes ? `${row.overtimeMinutes}m` : "—"}</td><td className="p-3">{row.distanceKm} km</td><td className="p-3">{row.tripExpenses ? `${currency(row.tripExpenses)}${row.tripPurpose ? ` · ${row.tripPurpose}` : ""}` : "—"}</td></tr>) : <tr><td colSpan={12} className="p-10 text-center text-muted-foreground">{loading ? "Loading report..." : "No attendance records for this month."}</td></tr>}</tbody></table></CardContent></Card>
  </div>;
}
