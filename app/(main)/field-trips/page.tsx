...
export default function FieldTripsPage() {
  ...
  return <div className="space-y-6 pb-10"><PageHeader title="Field Trips" />
    ...
    : <><Card className="border-blue-300"><CardHeader><CardTitle className="flex flex-wrap items-center justify-between gap-2"><span>{active.clientSiteId.clientName} · {active.clientSiteId.siteName}</span><Badge>{active.status.replaceAll("_", " ")}</Badge></CardTitle></CardHeader><CardContent className="space-y-4"><p>{active.source} → {active.destination}</p><p className="text-sm text-muted-foreground">{active.purpose} · {active.travelMode.replaceAll("_", " ")} · Expected return {new Date(active.expectedReturnAt).toLocaleString()}</p>
    ...
    <div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardTitle className="text-sm">Advance</CardTitle></CardHeader><CardContent className="text-2xl font-semibold">₹{active.advanceAmount.toFixed(2)}</CardContent></Card><Card><CardHeader><CardTitle className="text-sm">Expenses</CardTitle></CardHeader><CardContent className="text-2xl font-semibold">₹{active.totalExpenses.toFixed(2)}</CardContent></Card><Card><CardHeader><CardTitle className="text-sm">Balance</CardTitle></CardHeader><CardContent className={`text-2xl font-semibold ${balance < 0 ? "text-red-600" : ""}`}>₹{balance.toFixed(2)}</CardContent></Card></div>
