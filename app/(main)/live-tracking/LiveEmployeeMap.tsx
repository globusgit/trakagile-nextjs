...
export default function LiveEmployeeMap({ locations, selectedEmpId, onSelectEmployee, loading }: Props) {
  ...
  const triggerPoints = selected?.route.filter((point) => point.type === "TRIGGER" || point.type === "MARK_IN" || point.type === "MARK_OUT") || [];
    ...
    <div className="ops-workspace">
      <div className="ops-map-wrap">
        <MapContainer center={[selected.latitude, selected.longitude]} zoom={14} scrollWheelZoom zoomControl className="size-full">
          ...
          {selected.route.length > 1 && <><Polyline positions={selected.route.map((point) => [point.latitude, point.longitude])} pathOptions={{ color: "#38bdf8", weight: 10, opacity: .22 }} /><Polyline positions={selected.route.map((point) => [point.latitude, point.longitude])} pathOptions={{ color: "#0284c7", weight: 5, opacity: .95 }} /></>}
          {triggerPoints.map((point, index) => <Marker key={`${point.capturedAt}-${index}`} position={[point.latitude, point.longitude]} icon={triggerIcon(index + 1, point.type)}><Popup><strong>{point.type === "MARK_IN" ? "Marked in" : point.type === "MARK_OUT" ? "Marked out" : `Trigger ${index + 1}`}</strong><br />{point.locationName || "Location change detected"}<br />{time(point.capturedAt)}</Popup><Tooltip direction="top">{point.type === "TRIGGER" ? `Trigger ${index + 1}` : point.type.replace("_", " ")}</Tooltip></Marker>)}
          <Circle center={[selected.latitude, selected.longitude]} radius={90} pathOptions={{ color: "#0ea5e9", fillColor: "#38bdf8", fillOpacity: .1, weight: 1 }} />
          ...
          <Marker position={[selected.latitude, selected.longitude]} icon={liveIcon(selected.route.at(-1)?.heading || 0)}><Popup><strong>Live position</strong><br />{selected.locationName}<br />Last sync {time(selected.receivedAt)}</Popup><Tooltip permanent direction="top" offset={[0, -24]}>LIVE</Tooltip></Marker>
        </MapContainer>
...
function EmployeeDetails({ employee, triggerPoints, selectedStatus }: { employee: EmployeeLocation; triggerPoints: EmployeeLocation["route"]; selectedStatus: TrackStatus }) {
  ...
  return <div className="space-y-4">
    ...
    <div><p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Current location</p><div className="rounded-xl bg-sky-50 p-3 text-sm text-slate-700"><MapPin className="mb-2 size-4 text-sky-600" />{employee.locationName}</div></div>
    <div><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Today&apos;s route events</p><span className="text-xs font-semibold text-sky-600">{events.length} points</span></div><div className="ops-timeline">{events.map((point, index) => { const isLive = point.type === "LIVE"; return <div key={`${point.capturedAt}-${index}`} className="ops-event"><span className={isLive ? "live" : point.type === "MARK_IN" ? "start" : "trigger"}>{isLive ? <Navigation /> : point.type === "MARK_IN" ? <MapPin /> : index + 1}</span><span><strong>{isLive ? "Live position" : point.type === "MARK_IN" ? "Marked in" : point.type === "MARK_OUT" ? "Marked out" : `Location trigger ${index + 1}`}</strong><small>{point.locationName || "Location recorded"}</small></span><time>{time(point.capturedAt)}</time></div>; })}</div></div>
  </div>;
}
